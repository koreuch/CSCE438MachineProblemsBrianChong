...
class IClient
{
    public:
        void run_client() { run(); }
        // virtual void run_client_for_server() = 0;
...
void IClient::run()
{
    int ret = connectToCoordinator(); // we're setting up a connection with the coordinator first
    std::cout << "sucessfully got back from coordinator I thik" << std::endl;
    if (ret < 0) {
